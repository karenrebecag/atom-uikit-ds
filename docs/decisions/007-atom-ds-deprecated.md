...
- **Context:** Coexistían dos monorepos de design system: `atom-ds` (repo GitHub
  `karenrebecag/ATOM_DS`, scope `@atomchat.io/*`, lenguaje pre-OSMO con Inter como única
  ...
  de conformance de este repo (Major Third 1.25, base-4) y las specs de unificación OSMO
...
- **Consequences:**
  ...
  - Si aparece un consumidor Vue/Angular real, el wrapper se crea como package de este
    monorepo consumiendo `@atom-uikit/tokens`; nunca se revive `atom-ds` (su skin pre-OSMO
